...
- `pongo` — desktop (Ryzen 7 2700X / GTX 950); also runs the home-server stack (Nextcloud, Jellyfin/*arr media stack, Home Assistant, Zigbee2MQTT, Hercules CI agent, Docker).
...
- `hosts/flake-module.nix` — defines `flake.nixosConfigurations` (`pongo`, `tamarin`, `hetzner`) through a local `nixosSystemWithDefaults` helper. Every host automatically imports the `commonProfiles` set (agenix, cachix-deploy, docs, misc, networking, nix-nixpkgs, security, ssh, tailscale, users) plus `./<hostName>` and `networking.hostName` / `nixpkgs.hostPlatform`. Desktop hosts (`pongo`, `tamarin`) additionally import `commonHome` which loads `home-manager.nixosModules.default` and points `users.kranzes` at `home/kranzes`.
...
- `dev/flake-module.nix` — devshell, `treefmt`, the `deploy-<host>` apps, and the Hercules CI integration. The `herculesCI` block runs a Cachix Deploy effect on every push to `master`, with a rollback script that aborts rollback if `tailscale` is still up (treats Tailscale reachability as the liveness signal).
...
- File-name convention: `<host>-<purpose>.age` (or `all-…age` for cross-host secrets). `profiles/cachix-deploy.nix` and `hosted/restic.nix` resolve secret paths from `${config.networking.hostName}-…age`, so renaming a host requires renaming its secret files in lockstep.

### Deploy / cache pipeline

- Pushes to `master` trigger Hercules CI → Cachix Deploy (see `dev/flake-module.nix`). Each host's `cachix-agent` (configured in `profiles/cachix-deploy.nix`) pulls the new closure from `kranzes.cachix.org`. Manual rollouts use `nix run .#deploy-<host>` instead.
- The flake's `nixConfig` adds `kranzes.cachix.org` as a substituter; `nix-community.cachix.org` is added inside `profiles/nix-nixpkgs.nix`.
